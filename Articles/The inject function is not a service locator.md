...
author: Riegler Matthieu
source: https://riegler.fr/blog/2025-01-08-inject-not-service-locator
read: false
---
...
2. They may execute multiple times during a component's lifecycle
3. They operate in a runtime context rather than an initialization context
4. They're meant for lifecycle management, not dependency resolution

These design choices have a direct impact on app architecture. The aforementioned limitations enforce better architectural decisions by:

1. Ensuring dependencies are declared upfront
2. Maintaining clear dependency chains
3. Preventing runtime dependency resolution
4. Encouraging proper separation of concerns
...
4. Lifecycle management separation

Understanding these boundaries helps develop more maintainable and predictable Angular applications. Now that you understand in which context your are working when developing Angular app, let's take a look on why the `inject` function is Dependency Injection and not a Service Locator
...
Those functions rely on the injection context to have access to `DestroyRef` or other framework provided services. Whenever you invoked them outside of injection context, the framework will require you to provide a `injector` function. This is border service locator but yet acceptable because of the framework itself gives you the assurance that those services will always exist whenever you reach out to them. There is no risk that those dependencies are unmet in an injection context.
## Conclusion
...
When the feature is enabled, properties are initialized before the constructor it self run. Resulting error prone code.
...
- [_Inversion of Control Containers and the Dependency Injection pattern_, on martinFowler.com](https://martinfowler.com/articles/injection.html#ServiceLocator)  
  This is considered the canonical reference that first properly defined the pattern
- [_Service Locator is an Anti-Pattern_, Mark Seemann's critical analysis](https://blog.ploeh.dk/2010/02/03/ServiceLocatorisanAnti-Pattern/)
